...
// for each proof, update 2**m transactions
    ...
    // ==== k15n ==== //
    // Note on inputs:
    // Only the merkle roots of the balance and txn trees are public, whereas all
    // the transaction details are kept private.
    // This ensures the privacy of the transaction details on the L2 scaling soln.
    // This however compromises data availability, as the txn data is not stored
    // on-chain. The L2 needs to ensure that the txn data is stored securely and
    // its availability is ensured by setting up a Data Availability Committee to
    // ensure that users are always able to access their funds and perform txns.
    // ==== k15n ==== //

    // Merkle root of transactions tree
    //
    // ==== k15n ==== //
    // Public input. The root of the new transactions tree.
    // The smart contract consumes this and updates the Merkle tree stored on the
    // blockchain.
    // ==== k15n ==== //
    signal input txRoot;

    // ==== k15n ==== //
    // The next 2 are private inputs.
    // ==== k15n ==== //
    ...
    //
    // ==== k15n ==== //
    // Public input. The root of the merkle tree before the new txns are added.
    // The smart contract consumes this and checks that it is the same as the
    // merkle root stored on the blockchain.
    //
    // This ensures that the proof is generated from the correct start state, and that
    // the circuit did not spoof the old merkle root in order to incorrectly produce a
    // valid proof.
    // ==== k15n ==== //
    signal input currentState;

    // ==== k15n ==== //
    // The next are private inputs and have self explanatory comments.
    // ==== k15n ==== //
    
    // intermediate roots (two for each tx), final element is last.
    //
    // ==== k15n ==== //
    // For each transaction, two intermediate merkle tree roots are calculated
    // - one for the send part of the txn
    // - the second for the receive part of the transaction.
    // While checking the validity of each of the 2^m txns, the validity of the
    // intermediate merkle root is checked against this input array.
    // ==== k15n ==== //
    ...
    // ==== k15n ==== //
    // Check that the value of the intermediateRoots array at the 0th index is the same
    // as the merkle root of the old transactions merkle tree.
    // ==== k15n ==== //
    currentState === intermediateRoots[0];

    // ==== k15n ==== //
    // - Check the validity of the transaction and the signatures for each of the 2^m txns.
    // - Checks the validity of the sender and receiver account.
    // - Also checks each sender's account balance, nonce values and token types.
    // - Checks the merkle root of the balance tree after processing the send txn leaf.
    //   (the intermediateRoot check)
    // - Checks the receiver account's existence and updates the receiver's balance.
    // - Checks the merkle root of the balance tree after processing the receiver's
    //   balance (intermediateRoot check)
    // ==== k15n ==== //
    for (var i = 0; i < 2**m; i++) {
