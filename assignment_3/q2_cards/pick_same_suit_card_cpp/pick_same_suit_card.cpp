...
void MarkSuit_4_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void MarkSuit_4_run(uint ctx_index,Circom_CalcWit* ctx);
...
void IsSameSuit_7_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void IsSameSuit_7_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 2433;}

uint get_number_of_components() {return 54;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 1840;}
...
ctx->componentMemory[coffset].templateName = "MarkSuit";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 1;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "inRange1";
mySubcomponents[aux_create+i] = aux_cmp_num;
IsInRange_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 81 ;
aux_cmp_num += 6;
}
}
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "inRange2";
mySubcomponents[aux_create+i] = aux_cmp_num;
IsInRange_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 81 ;
aux_cmp_num += 6;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "inRange3";
mySubcomponents[aux_create+i] = aux_cmp_num;
IsInRange_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 81 ;
aux_cmp_num += 6;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "inRange4";
mySubcomponents[aux_create+i] = aux_cmp_num;
IsInRange_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 81 ;
aux_cmp_num += 6;
...
ctx->componentMemory[coffset].templateName = "IsSameSuit";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 3;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "multiRange1";
mySubcomponents[aux_create+i] = aux_cmp_num;
MarkSuit_4_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 329 ;
aux_cmp_num += 25;
}
}
...
uint csoffset = mySignalStart+2103;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "multiRange2";
mySubcomponents[aux_create+i] = aux_cmp_num;
MarkSuit_4_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 329 ;
aux_cmp_num += 25;
...
Fr_geq(&expaux[0],&signalValues[mySignalStart + 1],&circuitConstants[1]); // line circom 78
if (!Fr_isTrue(&expaux[0])) std::cout << "Failed assert in template " << myTemplateName << " line 78. " <<  "Followed trace: " << ctx->getTrace(myId) << std::endl;
assert(Fr_isTrue(&expaux[0]));
Fr_geq(&expaux[0],&signalValues[mySignalStart + 2],&circuitConstants[1]); // line circom 79
if (!Fr_isTrue(&expaux[0])) std::cout << "Failed assert in template " << myTemplateName << " line 79. " <<  "Followed trace: " << ctx->getTrace(myId) << std::endl;
assert(Fr_isTrue(&expaux[0]));
Fr_lt(&expaux[0],&signalValues[mySignalStart + 1],&circuitConstants[9]); // line circom 80
if (!Fr_isTrue(&expaux[0])) std::cout << "Failed assert in template " << myTemplateName << " line 80. " <<  "Followed trace: " << ctx->getTrace(myId) << std::endl;
assert(Fr_isTrue(&expaux[0]));
Fr_lt(&expaux[0],&signalValues[mySignalStart + 2],&circuitConstants[9]); // line circom 81
...
// need to run sub component
assert(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter));
MarkSuit_4_run(mySubcomponents[cmp_index_ref],ctx);
}
{
uint cmp_index_ref = 1;
{
...
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[16]); // line circom 89
while(Fr_isTrue(&expaux[0])){
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + ((1 * Fr_toInt(&lvar[0])) + 0)],&ctx->signalValues[ctx->componentMemory[mySubcomponents[1]].signalStart + ((1 * Fr_toInt(&lvar[0])) + 0)]); // line circom 90
if (!Fr_isTrue(&expaux[0])) std::cout << "Failed assert in template " << myTemplateName << " line 90. " <<  "Followed trace: " << ctx->getTrace(myId) << std::endl;
assert(Fr_isTrue(&expaux[0]));
{
PFrElement aux_dest = &lvar[0];
// load src
Fr_add(&expaux[0],&lvar[0],&circuitConstants[2]); // line circom 89
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[16]); // line circom 89
...
IsSameSuit_7_create(1,0,ctx,"main",0);
IsSameSuit_7_run(0,ctx);
}
